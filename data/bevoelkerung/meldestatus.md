---
title: Bevölkerung nach Meldestatus - Wohnberechtigte Bevölkerung und Wohnbevölkerung
source: Stadt Konstanz, Amt für Digitalisierung und IT (eigene Einwohnerfortschreibung)
order: 1
columns:
- name: jahr
  type: int
  description: Jahr
  short: Bevölkerung der Stadt Konstanz nach Meldestatus seit 1995
  export_id: Jahr
- name: wohnbevoelkerung_hauptwohnsitz
  type: int
  description: Hauptwohnsitz in Konstanz
  unit: Anzahl Personen
  export_id: Anzahl Einwohner mit Hauptwohnsitz in Konstanz
- name: wohnberechtigte_haupt_und_nebenwohnsitz
  type: int
  description: Haupt- oder Nebenwohnsitz in Konstanz
  unit: Anzahl Personen
  export_id: Anzahl Einwohner mit Haupt- oder Nebenwohnsitz in Konstanz
- name: wohnberechtigte_nebenwohnsitz
  type: int
  description: Nebenwohnsitz in Konstanz
  unit: Anzahl Personen
  export_id: Anzahl Einwohner mit Nebenwohnsitz in Konstanz
---
Anmerkungen:
Die Einwohnerzahl beruht auf der eigenen Fortschreibung zum Stichtag 31.12. basierend auf dem städtischen Melderegister. Die Wohnbevölkerung  umschreibt die "Bevölkerung am Ort der Hauptwohnung" (Einwohner mit Hauptwohnsitz). Die wohnberechtigte Bevölkerung umfasst alle in Konstanz gemeldeten EinwohnerInnen, sowohl die mit Hauptwohnung als auch die mit Nebenwohnsitz. Seit 2006 wurde die Zweitwohnungssteuersatzung mehrmals angepasst. Die Erhöhung des  Steuersatzes und Erstwohnsitz-Kampagnen führt zu einem deutlichen Rückgang der Nebenwohnsitze bis 2010.
